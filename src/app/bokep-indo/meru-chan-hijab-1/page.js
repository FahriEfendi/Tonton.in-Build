...
export default function meruchanhijab1({ videoId }) {
  ...
  const handleLikeClick = async () => {
    ...
    if (liked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/declike/4734da25-ae1c-48df-a39b-2f34f9cdea7e`, {
          method: 'PUT',
        });
        if (!response.ok) {
      ...
      if (dislike && dislike[videoId]) {
        ...
        try {
          const response = await fetch(`http://localhost:8080/videos/incdislike/4734da25-ae1c-48df-a39b-2f34f9cdea7e`, {
            method: 'PUT',
          });
          if (!response.ok) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/videos/inclike/4734da25-ae1c-48df-a39b-2f34f9cdea7e`, {
          method: 'PUT',
        });
        if (!response.ok) {
          throw new Error('Failed to like video');
  ...
  const handleDislikeClick = async () => {
    ...
    if (disliked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/decdislike/4734da25-ae1c-48df-a39b-2f34f9cdea7e`, {
          method: 'PUT',
        });
        if (!response.ok) {
  ...
  return (
    <div className="min-h-full bg-[#101633]">
      <Layout>
        <Helmet>
          <title>Meruchan Hijab Colmek - bokeptante.fun</title>
        </Helmet>
            ...
            <div className="mx-auto max-w-full sm:max-w-screen-lg py-6 sm:px-6 lg:px-8">
              <div className="relative h-0" style={{ paddingBottom: "56.25%" }}>
                <iframe
                  ...
                  src="https://gdriveplayer.uk/embed/?MnpORHdGN09tZzErRDFKUGNjR2hTekVlTGxiejJtM1pyV2cvVHRmSFRvTUR5QTNzajdIaUI5aXdHd2RlZURxellrQkE2cWcxRGcrWkRpNVVDbzZyQ25LaGpxOFVjaW5MV0JjZ1ZrV24wOEk9"
