...
function Dashboard() {
  ...
  const displayVideos = vidData && vidData.data.slice(pagesVisited, pagesVisited + videosPerPage)
    .map((video, index) => (
      <a
        key={index}
      ...
      >
        ...
        >
          <p className="text-sm text-white sm:text-base lg:text-lg xl:text-lg xl:text-sm  xl:text-slate-300 flex items-center " style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
            ...
            <span style={{ marginLeft: '0.25rem' }}>{video.views >= 1000000 ? `${(video.views / 1000000).toFixed(1)} m` : video.views >= 1000 ? `${(video.views / 1000).toFixed(1)} k` : video.views}</span>
          </p>
          <p className="text-sm text-white sm:text-base lg:text-sm xl:text-sm xl:text-sm  xl:text-slate-300 flex items-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
            ...
            <span style={{ marginLeft: '0.25rem' }}>{video.episode}</span>
          </p>
        </div>
        <div
